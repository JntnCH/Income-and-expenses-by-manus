...
function extractLINEUser(payload) {
  ...
  // LINE ไม่ส่ง displayName ใน webhook โดยตรง ต้องเรียก LINE API
  // แต่เก็บ userId ไว้เพื่อ lookup ภายหลัง
...
function extractFacebookUser(payload) {
  ...
  // Facebook ไม่ส่งชื่อใน webhook โดยตรง เก็บ PSID ไว้
...
function extractFromSession(body) {
  ...
  // session format: projects/{project}/agent/sessions/{session-id}
